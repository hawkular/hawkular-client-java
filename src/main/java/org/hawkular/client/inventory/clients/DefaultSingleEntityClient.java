...
public class DefaultSingleEntityClient extends BaseClient<SingleEntityHandler> implements SingleEntityClient {
    ...
    @Override
    public ClientResponse<Empty> deleteEntity(CanonicalPath path) {
        Response serverResponse = null;

        try {
            serverResponse = restApi().deleteEntity(path.toRelativePath().toString());
    ...
    @Override
    public ClientResponse<Map> getEntity(CanonicalPath path) {
        Response serverResponse = null;

        try {
            serverResponse = restApi().getEntity(path.toRelativePath().toString());
    ...
    @Override
    public ClientResponse<Empty> updateEntity(CanonicalPath path, AbstractElement.Update update) {
        Response serverResponse = null;

        try {
            serverResponse = restApi().updateEntity(path.toRelativePath().toString(), update);
            JavaType javaType = simpleResolver().get(Empty.class);
    ...
    @Override
    public ClientResponse<IdentityHash.Tree> getEntityHash(CanonicalPath path) {
        Response serverResponse = null;

        try {
            serverResponse = restApi().getEntityHash(path.toRelativePath().toString());
    ...
    @Override
    public ClientResponse<Map> createEntity(CanonicalPath path, SegmentType type, AbstractElement.Blueprint entity) {
        Response serverResponse = null;

        try {
            serverResponse = restApi().createEntity(path.toRelativePath().toString(), type.getSimpleName(), entity);
            JavaType javaType = simpleResolver().get(Map.class);
